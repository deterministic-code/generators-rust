...
pub struct MysqlStandardRepository {
...
impl MysqlStandardRepository {
    ...
    fn build_call_sql(proc_name: &str, params_len: usize) -> String {
        let placeholders: Vec<String> = (0..params_len).map(|_| "?".to_string()).collect();
    ...
    async fn invoke_returning_id(
        ...
        head.get("id").and_then(|v| v.as_i64()).ok_or_else(|| {
            RepositoryError::Other(format!(
                "{} did not return id in first recordset",
                proc_name
            ))
        })
    }
    ...
    async fn invoke_returning_affected(
        ...
        head.get("affected")
            ...
            .ok_or_else(|| {
                RepositoryError::Other(format!(
                    "{} did not return affected in first recordset",
                    proc_name
                ))
            })
    }
...
#[async_trait]
impl Repository for MysqlStandardRepository {
...
#[async_trait]
impl CrudRepository for MysqlStandardRepository {
    ...
    async fn add(&self, data: RowMap) -> Result<RowMap, RepositoryError> {
        ...
        let row = self
            ...
            .into_iter()
            .next()
            .ok_or(RepositoryError::InsertedRowMissing(id))?;
        Ok(self.apply_from_and_translate(row))
    }
...
impl StandardCrudRepository for MysqlStandardRepository {}
