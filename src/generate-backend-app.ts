...
import { appWiringComposePath } from "./rust-crate-paths.ts";
import {
...
export function generateMainRs({
...
export function generateLibRs({ byFeature = false }: { byFeature?: boolean } = {}) {
...
async function bundledRuntimeEntries(): Promise<GenerateEntry[]> {
...
function scaffoldPatchEntries(laneDir: string): GenerateEntry[] {
  return [
    {
      kind: PATCH,
...
export async function generateBackendApp(args: GenerateArgs): Promise<GenerateEntry[]> {
  ...
  const entries: GenerateEntry[] = skeletonEntriesFromFiles(
...
/** Catalog entry for the rust `backend_app` step. `rustModuleWiring = false`: backend_app generates `src/lib.rs` with an empty MODULES marker block that later per-module steps patch, so the generic mod.rs pass must not run. */
