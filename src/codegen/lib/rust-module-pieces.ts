...
function findSrcRoot(leafDir: string): string | null {
  let dir = leafDir;
  for (let i = 0; i < 12; i++) {
...
function toRustModIdent(fileBase: string): string {
  return fileBase.replace(/-/g, "_");
}

function isTestModule(base: string): boolean {
...
function isRouterModule(base: string): boolean {
  return (
    base === "router" || base.endsWith("_router") || base.endsWith("-router")
  );
}
...
function pathAddressed(srcRelDir: string): boolean {
  const top = srcRelDir.split("/")[0];
  return top === "features" || top === "routes";
}

function modDecl(base: string, { reexport }: { reexport: boolean }): string {
  const ident = toRustModIdent(base);
  const decl =
    VALID_RUST_IDENT.test(ident) && ident === base
      ? `pub mod ${ident};`
      : `#[path = "${base}.rs"] pub mod ${ident};`;
  if (isTestModule(base)) return `#[cfg(test)]\nmod ${ident};`;
...
function cratePathFor(srcRelDir: string, base: string): string {
...
function pieceEntry(target: string, content: string, section?: string): Piece {
...
function piecesForFile(srcRelPath: string, custom?: CustomService): Piece[] {
  const dir = posix.dirname(srcRelPath);
  if (dir === ".") return [];
  const base = posix.basename(srcRelPath, ".rs");
  const out: Piece[] = [
    pieceEntry(
      posix.join(dir, "mod.rs"),
      modDecl(base, { reexport: !pathAddressed(dir) }),
    ),
  ];
...
export function isWireableRustFile(srcRelPath: string): boolean {
  const b = posix.basename(srcRelPath);
  if (b === "mod.rs" || b === "lib.rs" || b === "main.rs") return false;
  if (!b.endsWith(".rs")) return false;
  const dir = posix.dirname(srcRelPath);
  if (dir === ".") return false;
  if (dir.split("/")[0] === "bin") return false;
  return true;
}
 ...
 */
export function rustModuleEntriesFrom({
  outDir,
  files,
}: {
  outDir: string;
  ...
  for (const f of files) {
    ...
    if (toPosix(abs).includes("/_deterministic/")) continue;
    const srcRel = toPosix(relative(srcRoot, abs));
    if (srcRel.startsWith("..") || !isWireableRustFile(srcRel)) continue;
    ...
    if (/(^|\/)custom$/.test(posix.dirname(srcRel))) {
      ...
      if (struct) customServices.set(srcRel, { struct });
    }
  }
