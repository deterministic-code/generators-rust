...
interface RustEmitOpts extends DatasourceOptions {
...
function inheritedDatasourceFields(
  ...
  if (!entry) {
    ...
    throw new Error(
      `emit-view-rust: view inherits datasource_types.${tableName} but no such type in datasource. Available: ${available || "(none)"}.`,
    );
  }
...
function emitInlinedField(field: DeclaredField, ctx: RustCtx): string {
  const rustType =
    field.name === "id"
...
function emitField(field: ViewField, ctx: RustCtx): string {
