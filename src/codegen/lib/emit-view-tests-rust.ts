...
function pushFieldLine(args: {
  ...
  if (emitted.has(fieldName)) {
    throw new Error(
      `emit-view-tests-rust: duplicate field "${fieldName}" in struct literal for view "${viewName}". ` +
...
export function emitForView(
  view: View,
  {
    datasource,
    viewIndex,
  }: { datasource: Datasource; viewIndex: Map<string, View> },
  options: Partial<RustEmitOptions> = {},
): EmittedFile {
...
export function emitFromSchema(
...
export const createEmitter = () => {
  ...
  return {
    emit: (config: { settings: ParsedSettings; language: string }) =>
      base.emit({
