...
function typeName(name: string, opts: RustEmitOptions): string {
...
function fieldCase(name: string, opts: RustEmitOptions): string {
  return toCase(name, opts.fieldFormat as CaseFormat); // lint-emitter-casing-allow: toCase
}

function fileBase(name: string, opts: RustEmitOptions): string {
  return toCase(`${name}_validator_tests`, opts.fileFormat as CaseFormat); // lint-emitter-casing-allow: toCase
}
...
function allFields(tableDef: RustTableDef, opts: RustEmitOptions): RustField[] {
...
export function emitForTable(
  entry: Record<string, unknown>,
  datasource: unknown,
  options: Partial<RustEmitOptions> = DEFAULT_EMIT_OPTIONS,
): { path: string; content: string } {
  const opts: RustEmitOptions = { ...DEFAULT_EMIT_OPTIONS, ...options };
