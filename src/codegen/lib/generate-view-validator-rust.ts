...
function generateShapedValidator(view: ShapedView, ctx: RustValidatorCtx): string {
...
function generateUnionValidator(view: UnionView, ctx: RustValidatorCtx): string {
  const cls = ctx.imports.viewType(view.name);
  const fn = viewValidatorFn(view.name);
