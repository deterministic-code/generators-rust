...
export function emitPerfServerRust({
  datasourceData,
}: { datasourceData?: DatasourceData } = {}): EmittedFile {
  if (!datasourceData || typeof datasourceData !== "object") {
    throw new Error(
      "emitPerfServerRust: datasourceData is required (parsed datasource_types.yaml)",
    );
  }
  if (!Array.isArray(datasourceData.types)) {
    throw new Error(
      "emitPerfServerRust: datasourceData.types must be an array — malformed datasource_types.yaml",
    );
  }
...
export async function emit({ inputs, settings }: PerfServerRustEmitInput) {
  const { datasourceYamlText } = await inputs.all();
  const file = emitPerfServerRust({
