...
function emitSetupFn(entityName: string): string {
...
function emitRegularEntityTests(
  entityName: string,
  typeDef: TypeDef,
...
function emitReadonlyEntityTests(
  entityName: string,
  missingSegment: string,
): string {
  const pascal = pascalize(entityName);
...
export function emitAppE2ETest({
  datasourceData,
  datasourceSettings,
}: E2ETestInput): EmittedFile {
  ...
  routesStepEmit(
    {
      ...
      emitter: { createEmitter } as unknown as LanguageEmitterModule,
      language: "rust",
    },
    ctx,
  );
...
export const createEmitter = () => ({
  emit: (config: RoutesEmitConfig & { expanded: unknown }) =>
    emitRouteE2eFiles({ ...config, primitives: { emitAppE2ETest } }),
});
