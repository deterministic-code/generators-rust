// Rust `create_backend_app` generator: src/main.rs boots the runtime's dynamic server (deterministic::run(RunConfig::from_env())) plus the surrounding scaffold (Cargo.toml / Dockerfile / docker-compose / entrypoint / .env / .env.example / .gitignore). The previous health-only stub main + APP_* marker patching was deleted: the patch silently missed in synthesized crates and stub servers shipped to production.
import { readdir, readFile } from "node:fs/promises";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { SECTION_MARKERS } from "@deterministic-code/generator-sdk/section-markers";
import { RUST_APP_DEPS, RUST_APP_DEV_DEPS } from "./rust-deps.js";
import { DEV_PORTS } from "@deterministic-code/generator-sdk/create-backend-app-model";
import { makeChunkLoader, applyTokens, } from "@deterministic-code/generator-sdk/codegen/lib/chunk-loader";
import { PACK_ROOT, PACK_TEMPLATES_DIR } from "../pack-root.js";
...
import { COMPOSE_FILENAME, renderRustStandaloneComposeService, renderRustComposeService, } from "@deterministic-code/generator-sdk/codegen/lib/compose-services";
...
import { appWiringComposePath } from "../codegen/lib/rust-crate-paths.js";
...
import { CONTENT, PATCH, skeletonEntriesFromFiles, } from "@deterministic-code/generator-sdk/codegen/lib/generate-result";
import { readSettingsWithDefault, resolveLibraryReferenceMode, } from "@deterministic-code/generator-sdk/read-settings";
const here = dirname(fileURLToPath(import.meta.url));
const RUST_ENTRYPOINT_TEMPLATE_PATH = resolve(PACK_TEMPLATES_DIR, "create-backend-app", "rust", "entrypoint.sh");
...
/** Lib + bin target name of every generated consumer crate; main.rs (bin) references generated modules through it. The runtime dep keeps its natural name `deterministic`, so generated code resolves it from the extern prelude everywhere — no extern-crate alias. */
export const RUST_SYNTHESIZED_CRATE_NAME = "generated";
export function generateMainRs({ packageName = RUST_SYNTHESIZED_CRATE_NAME, } = {}) {
...
export function generateLibRs({ byFeature = false } = {}) {
...
export const DEFAULT_REGISTRY_INDEX_URL = "sparse+https://deterministic-code.com/registry/";
export function renderCargoToml({ packageName = "generated", libraryReferenceMode = "bundled", crateVersion = null, registryName = DEFAULT_REGISTRY_NAME, } = {}) {
    ...
    return (applyTokens(CARGO_TOML_CHUNK, {
        packageName,
        detDep,
...
function renderDeterministicDep({ libraryReferenceMode, crateVersion, registryName, }) {
    if (libraryReferenceMode === "registry") {
        if (!crateVersion) {
            throw new Error("renderCargoToml: libraryReferenceMode=registry requires a crateVersion (read from the library's rust/Cargo.toml at codegen time)");
        }
        return `deterministic = { version = "= ${crateVersion}", registry = "${registryName}" }`;
    }
    return `deterministic = { path = "_deterministic/rust" }`;
}
/** why .cargo/config.toml at crate root: cargo discovers alternative registries via this file (or a global ~/.cargo/config.toml). Generating it sibling to Cargo.toml makes the consumer crate self-contained — `cargo build` in a fresh clone of the generated project finds the registry without any out-of-band setup. */
export function renderRustCargoConfig({ registryName = DEFAULT_REGISTRY_NAME, indexUrl = DEFAULT_REGISTRY_INDEX_URL, } = {}) {
...
export function renderRustDockerfile({ binaryName = "generated", libraryReferenceMode = "bundled", multiLanguage = false, laneDir, } = {}) {
    ...
    const sourceCopy = libraryReferenceMode === "registry"
        ? `COPY ${rustPrefix}.cargo ./.cargo`
        : `COPY ${rustPrefix}_deterministic ./_deterministic`;
    return (applyTokens(DOCKERFILE_CHUNK, {
        binaryName,
        sourceCopy,
        rustPrefix,
...
async function renderRustEntrypoint() {
    return await readFile(RUST_ENTRYPOINT_TEMPLATE_PATH, "utf8");
}
function renderRustEnvSection() {
    return `PORT=${DEV_PORTS.rust}
DETERMINISTIC_DIR=./deterministic
`;
}
function renderRustGitignoreSection() {
    return `target/
_deterministic/rust/target/
`;
}
export async function readLibraryCrateVersion() {
    ...
    if (!m) {
        throw new Error(`readLibraryCrateVersion: could not parse a version line out of ${cargoPath}`);
    }
    return m[1];
}
/** why env-driven (not CLI flags): the e2e spec needs to point the generated .cargo/config.toml at an in-process registry whose URL is only known at test-time (free-port allocation), and inject `host.docker.internal:host-gateway` so cargo inside the container reaches the host backend. Production deploys ignore both — defaults stay https://deterministic-code.com/registry and no extra_hosts. */
function registryConfigOverridesFromEnv() {
    ...
    const opts = {};
    if (indexUrl)
        opts.indexUrl = indexUrl;
    if (registryName)
        opts.registryName = registryName;
    return opts;
}
function dockerExtraHostsFromEnv() {
...
async function buildRustCrateFiles({ libraryReferenceMode, multiLanguage, laneDir, byFeature, }) {
    const crateVersion = libraryReferenceMode === "registry"
        ? await readLibraryCrateVersion()
        : null;
    const registryOverrides = registryConfigOverridesFromEnv();
    const files = [
        generateMainRs(),
        generateLibRs({ byFeature }),
        {
            path: "Cargo.toml",
...
async function resolveLibraryRustDir(rootDir) {
    return firstExistingDir([
        join(rootDir, "node_modules", "@deterministic-code", "deterministic", "rust"),
        join(rootDir, "rust"),
...
function rustBundleExclude(relativeFromSrc) {
    if (relativeFromSrc === "target")
        return true;
    if (relativeFromSrc.startsWith("target/"))
        return true;
    if (relativeFromSrc === "tests")
        return true;
    if (relativeFromSrc.startsWith("tests/"))
        return true;
    if (relativeFromSrc === "Cargo.lock")
        return true;
    if (relativeFromSrc.endsWith(".sqlite"))
        return true;
    if (relativeFromSrc.endsWith(".db"))
        return true;
    return false;
}
/** Cargo.toml / Dockerfile / entrypoint.sh / lib.rs are composed skeletons — backend_app generates the base piece (with MIGRATE_BIN/… or the MODULES/CUSTOM_SERVICES markers) and the per-module/migrate/perf steps fill the marked regions; everything else is single-contributor content. */
...
/** The single-contributor scaffold patches (compose service / .env / .gitignore / .dockerignore trigger) every rust backend app generates alongside its composed-skeleton crate files. */
function scaffoldPatchEntries(laneDir) {
    return [
        {
            kind: PATCH,
...
export async function generateBackendApp(args) {
    ...
    const libraryReferenceMode = resolveLibraryReferenceMode(settings.languages, "rust");
    ...
    const entries = skeletonEntriesFromFiles(await buildRustCrateFiles({
        libraryReferenceMode,
        multiLanguage,
        laneDir,
        byFeature,
...
export function buildRustCargoTomlDepsBlock(dialects = []) {
...
function rustDialectPoolTypes(dialects = []) {
    const has = (d) => dialects.includes(d);
    const parts = [];
    if (has("sqlite"))
        parts.push("SqlitePool");
    if (has("postgres"))
        parts.push("PgPool");
    if (has("mysql"))
        parts.push("MySqlPool");
    return parts;
}
export function rustDialectUseImportsForSetup(dialects = []) {
    return `use sqlx::{${rustDialectPoolTypes(dialects).join(", ")}};`;
}
export function rustDialectUseImportsForRunners(dialects = []) {
...
export function rustDialectSqliteUrlHelperBlock(dialects = []) {
...
export function rustDialectDdlConstsBlock(dialects = []) {
...
const RUST_SETUP_DISPATCH_CHUNKS = (await renderDialectMap("rust", "setup_dispatch", RUST_SETUP_DISPATCH_TOKENS));
export function rustDialectSetupDispatchBlock(dialects = []) {
...
export function rustDialectRunnerFnsBlock(dialects = []) {
...
const RUST_UP_DISPATCH_CHUNKS = (await renderDialectMap("rust", "up_dispatch", RUST_DIALECT_ONLY_TOKENS));
export function rustDialectUpDispatchBlock(dialects = []) {
...
export function rustDialectRollbackFnsBlock(dialects = []) {
    return filterChunks(RUST_ROLLBACK_FN_CHUNKS, dialects, "\n\n");
}
const RUST_DOWN_DISPATCH_CHUNKS = (await renderDialectMap("rust", "down_dispatch", RUST_DIALECT_ONLY_TOKENS));
export function rustDialectDownDispatchBlock(dialects = []) {
