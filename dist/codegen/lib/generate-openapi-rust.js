...
function entityPathBase(entityName) {
    return `/api/${layout.apiPath(entityName)}`;
}
function tagFor(entityName) {
    return layout.apiPath(entityName);
}
function collectionOps(tag) {
    return {
        get: {
            tags: [tag],
            summary: `List ${tag}`,
...
function buildEntityPaths(entityName) {
...
export function buildOpenApiSpec({ title = "API", version = "0.0.0", entities = [], enrichedSpec = null, } = {}) {
    if (enrichedSpec && typeof enrichedSpec === "object") {
        const clone = JSON.parse(JSON.stringify(enrichedSpec));
...
function conformancePaths(opts) {
...
export function generateOpenApiConformanceTest(opts = {}) {
...
export function generateOpenApiRouter({ title, version, entities = [], enrichedSpec = null, } = {}) {
