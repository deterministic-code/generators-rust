...
function generateCoerceRowFn(booleanFields, binaryFields) {
...
function generateCrudRouterContent(args) {
...
function generateCrudRouterPlain(ctx) {
...
function generateReadOnlyRouterContent(name, byFields, opts) {
    const entitySnake = name;
    const className = names.className(name, "service");
    const serviceImport = serviceUseLine(name, className, opts);
    const path = `/api/${layout.apiPath(name)}`;
...
export function generateReadOnlyRouter(candidate, options = {}) {
    ...
    return {
        ...
        content: generateReadOnlyRouterContent(candidate.name, Array.isArray(candidate.byFields) ? candidate.byFields : [], {
            byFeature,
            layout,
