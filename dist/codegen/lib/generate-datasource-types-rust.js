...
import { rustInjectedSystemFields } from "./rust-standard-columns.js";
...
export function normalizeIdType(idType) {
    if (idType === "i32")
        return "i32";
    return datasourceSettingsFor({ idType }).rustIdType();
}
function mapType(field, idType, datetime) {
    if (field.name === "id")
...
function normalizeTable(entry) {
    ...
    const declared = def.fields.map((f) => {
