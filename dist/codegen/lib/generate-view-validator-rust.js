...
import { buildViewGenerator, classifyViewShape, renderByViewKind, } from "@deterministic-code/generator-sdk/codegen/lib/generate-view-shared";
...
function viewValidatorFn(name) {
    return `validate_${toCase(name, "Snake")}`; // lint-generator-casing-allow: toCase
}
...
function nestedFnFor(parsed, ctx) {
    if (parsed.kind === "datasource")
        return ctx.imports.dsValidator(parsed.base);
...
function generateShapedValidator(view, ctx) {
    ...
    const parentCall = view.inherits && !inlineParent && !inlineForOmit
...
function generateUnionValidator(view, ctx) {
    const cls = ctx.imports.viewType(view.name);
    const fn = viewValidatorFn(view.name);
    const arms = view.members
        .map((m) => {
        const variant = toCase(m, "Pascal"); // lint-generator-casing-allow: toCase
...
function renderView(view, ctx) {
    ...
    const body = renderByViewKind(view, ctx, {
        shaped: (v, c) => generateShapedValidator(v, c),
        union: (v, c) => generateUnionValidator(v, c),
    });
