...
import { inlinedViewAuditFieldsExcluding } from "./rust-standard-columns.js";
import { viewEmitter } from "@deterministic-code/generator-sdk/codegen-context";
import { RustImports } from "./rust-imports.js";
import { datasourceOptionsFromSettings } from "@deterministic-code/generator-sdk/codegen/lib/emit-settings-options";
import { datasourceSettingsFor, } from "@deterministic-code/generator-sdk/codegen/lib/ts-datasource-settings";
import { buildViewEmitter, classifyViewShape, declaredFieldsOf, shapedViewDocLines, unionViewDocLines, } from "@deterministic-code/generator-sdk/codegen/lib/emit-view-shared";
...
function inheritedDatasourceFields(datasource, tableName, ds) {
    const entry = datasource.types.find((e) => Object.keys(e)[0] === tableName);
    if (!entry) {
        const available = datasource.types.map((e) => Object.keys(e)[0]).join(", ");
        throw new Error(`emit-view-rust: view inherits datasource_types.${tableName} but no such type in datasource. Available: ${available || "(none)"}.`);
    }
...
function rustTypeForField(field, ctx) {
    const { parsed } = field;
    let base;
    switch (parsed.kind) {
        case "primitive":
...
function inlineFieldsBlock(view, omit, ctx) {
    const parentFields = inheritedDatasourceFields(ctx.opts.datasource, view.inherits, datasourceSettingsFor(ctx.opts)).filter((f) => !omit.has(f.name));
    const lines = [
...
function viewFieldsBlock(view, ctx, { enrichments, inlineParent, inlineForOmit, omitList }) {
    const { imports } = ctx;
    if (inlineParent) {
        return inlineFieldsBlock(view, new Set(enrichments.map((e) => e.fkColumn)), ctx);
    }
    if (inlineForOmit) {
    ...
    const baseField = view.inherits
        ...
        : "";
    const body = view.fields.map((f) => emitField(f, ctx)).join("\n");
...
function renderView(view, ctx) {
    ...
    const { enrichments, inlineParent, inlineForOmit, omitList, aliasParent } = classifyViewShape(view);
    if (aliasParent) {
